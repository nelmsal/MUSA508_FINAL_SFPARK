...
X-Parks-the-Spot is an app that finds the perfect parking spot for the user. The perfect parking spot, to put it bluntly, is the that's availible closest to the user's destinaton. Currently, the app only operates in the pilot city of San Francisco, due to the city's availibility of data. 
...
3. time period of the day.

The development team formed a prediction model based on 2019 parking meter transactions in the City of San Francisco. Specifically, this is a binomial logistic model that predicts if the block's occupancy rate -- e.g. the aggregated occupation time of all the block's parking meters over the aggregated possible time -- will be lower or higher than 80%. 80% is the preferred threshold based on parking literature and that it is rate that San Francisco's parking project (SF Park) uses to price meters.

The model is fitted on the observed parking meter transactions of 2019's blocks . A few predictor variables to help predict the occupation.

*Why would someone want to replicate your analysis? Why would they use this approach?*

This approach is helpful as it gives the user a probability for their likelihood to find parking.
...
1. prepare data that gives us the occupancy rates for blocks of meters by time of day, 
